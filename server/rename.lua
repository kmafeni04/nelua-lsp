  ...
  if found_nodes then
    ...
    switch(true, {
      ...
      [current_node.is_DotIndex or false] = function()
        ...
        if current_node[2] and type(current_node[2]) == "table" then
          -- local scope_node = current_node[2].attr.scope.node
          -- local scope_nodes, err = find_nodes_in_scope_node(scope_node, scope_node.endpos, {})
          local s_line, s_char = pos_to_line_and_char(current_node.pos + 1, current_file)
          table.insert(changes[request_params.textDocument.uri], {
            ---@type Range
            range = {
              start = {
          ...
          elseif parent_node.attr.value then
